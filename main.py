...
# prevenir o erro do placeholder abaixo
tf.compat.v1.disable_eager_execution()

# a partir daqui construimos o modelo

# placeholders se tratam de variaveis que terão seu valores informados mais tarde,
# x representa os dados e y a clase qeu pertence dada flor
X = tf.compat.v1.placeholder("float", [None, n_input])
Y = tf.compat.v1.placeholder("float", [None, n_output])


# agora sao definidos variaveis referentes aos pesos e bias da rede
weights = {
...
# aqui ocorrem a multiplicação entre os valores da matriz
...
optimizador = tf.compat.v1.train.AdamOptimizer(learning_rate).minimize(cost)

init = tf.compat.v1.global_variables_initializer()


# aqui é feito a criação de uma seção, que é um ambiente onde as operações são realizadas e então é inicializado
# todas as variaveisutilizadas no modelo
with tf.compat.v1.Session() as sess:
    sess.run(init)

    # loop onde ocorrerão todas etapas de treinamento
